		...
		Context("for 3♦ (29)", func() {
			It("should return a valid slice of cards", func() {
				...
				* 6: 48 | 9♠
				...
				row, err := mm[0].Decks.Main.CalcHRow(card)
		...
		Context("for 3♦ (29)", func() {
			It("should return a valid slice of cards", func() {
				...
				* 5: 48 | 9♠
				...
				card, _ := core.NewCardFromNumber(29, locale)
				row, err := mm[0].Decks.Main.CalcVRow(card)
			...
			It("should not raise an error", func() {
				for i := uint8(0); i < 90; i++ {
					for j := uint8(1); j <= 52; j++ {
						...
						row, err := mm[i].Decks.Main.CalcVRow(c)
